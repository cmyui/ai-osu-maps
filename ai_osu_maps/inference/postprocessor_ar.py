...
from ai_osu_maps.inference.timing_points_change import TimingPointsChange, sort_timing_points
...
def position_to_progress(slider_path: SliderPath, pos: np.ndarray) -> float:
    ...
    for _ in range(100):
        grad = np.linalg.norm(
            slider_path.position_at(t) - pos
        ) - np.linalg.norm(slider_path.position_at(t - eps) - pos)
        t -= lr * grad
...
class Postprocessor:
        ...
        for group in groups:
            ...
            if group.distance is not None and group.x is None and group.y is None:
                coords = calculate_coordinates(
                    (last_x, last_y), group.distance, 500, (512, 384)
                )
            ...
            if hit_type in (
                EventType.CIRCLE, EventType.SLIDER_HEAD,
                EventType.BEZIER_ANCHOR, EventType.PERFECT_ANCHOR,
                EventType.CATMULL_ANCHOR, EventType.RED_ANCHOR,
                EventType.LAST_ANCHOR, EventType.SLIDER_END,
            ):
            ...
            if hit_type == EventType.CIRCLE:
                ...
                tp = self._timing_point_at(
                    timedelta(milliseconds=slider_start_time), timing
                )
                ...
                req_length = max_length * position_to_progress(
                    slider_path, np.array((group.x, group.y))
                )
                ...
                sv, length = self._get_human_sv_and_length(
                    req_length, max_length, span_duration,
                    last_sv, redline, slider_head.new_combo,
                    beatmap_config.slider_multiplier,
                )

                if length > max_length * 1.5:
                    sv = (
                        max_length / 100 / span_duration
                        * redline.ms_per_beat / beatmap_config.slider_multiplier
                    )
                    sv = round(sv * 20) / 20
                    length = self._calc_length(
                        sv, span_duration, redline, beatmap_config.slider_multiplier
                    )
                ...
                if sv != last_sv:
                    timing = self._set_sv(
                        timedelta(milliseconds=slider_start_time), sv, timing
                    )

                node_hitsounds = (
                    slider_head.hitsounds
                    + last_anchor.hitsounds[1:]
                    + group.hitsounds
                )
                node_samplesets = (
                ...
                node_additions = (
                    slider_head.additions
                    + last_anchor.additions[1:]
                    + group.additions
                )
                node_volumes = (
                    slider_head.volumes
                    + last_anchor.volumes[1:]
                    + group.volumes
                )

                body_hitsound = last_anchor.hitsounds[0] if last_anchor.hitsounds else 0
                body_sampleset = last_anchor.samplesets[0] if last_anchor.samplesets else 0
                ...
                node_hs_str = "|".join(map(str, node_hitsounds)) if node_hitsounds else "0"
                node_ss_str = (
                    "|".join(f"{s}:{a}" for s, a in zip(node_samplesets, node_additions))
                    if node_samplesets
                    else "0:0"
                )
                ...
                for i in range(min(slides + 1, len(node_volumes))):
                    ...
                    timing = self._set_volume(
                        timedelta(milliseconds=t), node_volumes[i], timing
                    )
                ...
                timing = self._set_kiai(
                    timedelta(milliseconds=group.time), bool(group.value), timing
                )
    ...
    def generate_timing(self, events: list[Event]) -> list[TimingPoint]:
        ...
        for time, _, is_redline in markers:
            if is_redline:
                tp = TimingPoint(
                    timedelta(milliseconds=time), 1000, 4, 2, 0, 100, None, False
                )
                ...
                if beat_count > 0:
                    ...
                    if mpb > 0:
                        redline = self._timing_point_at(
                            timedelta(milliseconds=last_redline_time), timing
                        )
    ...
    def _timing_point_at(
        time: timedelta, timing_points: list[TimingPoint]
    ) -> TimingPoint:
    ...
    @staticmethod
    def _set_volume(
        time: timedelta, volume: int, timing: list[TimingPoint]
    ) -> list[TimingPoint]:
    ...
    @staticmethod
    def _set_sv(
        time: timedelta, sv: float, timing: list[TimingPoint]
    ) -> list[TimingPoint]:
        if sv == 0:
            return timing
    ...
    @staticmethod
    def _set_kiai(
        time: timedelta, kiai: bool, timing: list[TimingPoint]
    ) -> list[TimingPoint]:
    ...
    def _get_human_sv_and_length(
        ...
        sv = (
            req_length / 100 / span_duration
            * redline.ms_per_beat / slider_multiplier
        )
        ...
        adjusted_length = self._calc_length(
            sv, span_duration, redline, slider_multiplier
        )
    ...
    def write_result(result: str, output_path: str) -> str:
        ...
        osu_path = os.path.join(
            output_path, f"beatmap{uuid.uuid4().hex}{OSU_FILE_EXTENSION}"
        )
