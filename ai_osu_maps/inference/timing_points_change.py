...
def same_effect(tp: TimingPoint, other: TimingPoint):
    return (
        tp.ms_per_beat == other.ms_per_beat
        and tp.meter == other.meter
        and tp.sample_type == other.sample_type
        and tp.sample_set == other.sample_set
        and tp.volume == other.volume
        and ((tp.parent is None) == (other.parent is None))
        and tp.kiai_mode == other.kiai_mode
    )
...
def compare_timing_point(tp1: TimingPoint, tp2: TimingPoint) -> int:
    ...
    return -(
        ((tp1.parent is None) > (tp2.parent is None))
        - ((tp1.parent is None) < (tp2.parent is None))
    )
...
class TimingPointsChange:
    ...
    def add_change(
        ...
        for tp in timing:
            ...
            if tp.offset < self.my_tp.offset and (
                prev_timing_point is None or tp.offset >= prev_timing_point.offset
            ):
                prev_timing_point = tp
            if math.isclose(
                tp.offset.total_seconds(),
                self.my_tp.offset.total_seconds(),
                abs_tol=self.fuzzyness,
            ):
        ...
        if not self.uninherited and (
            not on_timing_points or (self.ms_per_beat and not on_has_green)
        ):
            ...
            if prev_timing_point is None:
                ...
                adding_timing_point = copy(prev_timing_point)
                adding_timing_point.offset = self.my_tp.offset
                adding_timing_point.parent = (
                    prev_timing_point
                    if prev_timing_point.parent is None
                    else prev_timing_point.parent
                )
                if prev_timing_point.parent is None:
        ...
        if adding_timing_point and (
            prev_timing_point is None
            or not same_effect(adding_timing_point, prev_timing_point)
            or self.uninherited
        ):
    ...
    def apply_changes(
        timing: list[TimingPoint],
        timing_points_changes: list["TimingPointsChange"],
        all_after: bool = False,
    ) -> list[TimingPoint]:
    ...
    def debug(self):
        ...
        print(
            f"{self.ms_per_beat}, {self.meter}, {self.sample_type}, {self.sample_set}, {self.volume}, {self.uninherited}, {self.kiai_mode}",
        )
